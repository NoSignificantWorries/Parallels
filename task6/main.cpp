...
int main(int argc, char *argv[])
{
    ...
    #pragma acc data copyin(error, u_1[0:N*N], u_2[0:N*N])
    {
        ...
        while (iteration < max_iterations && error > tolerance)
        {
            ...
            if ((iteration + 1) % error_calc_step == 0)
            {
                ...
                #pragma acc parallel loop independent collapse(2) reduction(max:error) present(u_1, u_2) async(2)
                for (int i = 1; i < N - 1; ++i)
                {
                    for (int j = 1; j < N - 1; ++j)
                    {
                        error = fmax(error, fabs(u_2[OFFSET(i, j, N)] - u_1[OFFSET(i, j, N)]));
                    }
                }
                #pragma acc update self(error) async(2)
                #pragma acc wait(2)
